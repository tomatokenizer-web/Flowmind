...
import { Layers, Plus, Sparkles, Merge, Scissors, ChevronDown, ChevronRight, Loader2 } from "lucide-react";
...
export function ContextTree({ projectId, collapsed }: ContextTreeProps) {
  ...
  const suggestionsQuery = api.ai.suggestContextOperations.useQuery(
    { projectId: projectId! },
    { enabled: showSuggestions && !!projectId, refetchOnWindowFocus: false },
  );
            ...
            <div className="px-space-3 pb-space-2 space-y-space-1">
              ...
              {suggestionsQuery.data?.mergeSuggestions.map((m, i) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setMergeSourceId(m.contextIdA);
                    setMergeTargetId(m.contextIdB);
                  }}
                  className={cn(
              ...
              {suggestionsQuery.data?.splitSuggestions.map((s, i) => (
                <button
                  key={`split-${i}`}
                  type="button"
                  onClick={() => setSplitTargetId(s.contextId)}
                  className={cn(
                    "w-full rounded-md border border-border bg-bg-surface p-space-2 text-left",
                    "hover:bg-bg-hover transition-colors duration-fast",
                  )}
